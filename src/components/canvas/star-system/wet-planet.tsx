...
const randomizeColors = () => {
  ...
  const landColors = new Array(4).fill(0).map((_, i) => {
    const color = darken(seedColors[0]!, i / 4).offsetHSL(0.2 * (i / 4), 0, 0);
    return new Vector4(...color, 1);
  });

  const riverColors = new Array(2).fill(0).map((_, i) => {
    const color = darken(seedColors[1]!, i / 2).offsetHSL(0.2 * (i / 2), 0, 0);
    return new Vector4(...color, 1);
  });

  const cloudColors = new Array(4).fill(0).map((_, i) => {
    const color = lighten(seedColors[2]!, (1 - i / 4) * 0.8).offsetHSL(
      0.2 * (i / 4),
      0,
      0,
    );
