...
float rand(vec2 coord) {
  // GLSL ES may not provide round(); use floor(size + 0.5) and ensure >= 1.0
...
float turbulence(vec2 uv) {
  ...
  for (int i = 0; i < 10; i++) {
    c_noise += circleNoise(
      uv * size * 0.3 + (float(i + 1) + 10.0) + vec2(time * time_speed, 0.0)
    );
  }
  return c_noise;
}
...
void main() {
  ...
  float fbm2 = fbm(
    uv * vec2(1.0, 2.0) * size + fbm1 + vec2(-time * time_speed, 0.0) + turb
  );
